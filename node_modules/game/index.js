...
GameServer.prototype.RemoveProjectiles = function() {
    for (var i = this.clients.length; i--; ) {
        for (var j = this.clients[i].projectiles.length; j--; ) {
            ...
            {
                if ( !(this.clients[i].projectiles[j].type.indexOf("-rocket") > 0) )
                {
                    projectilePool.ReleaseProjectile(this.clients[i].projectiles[j]);
                }
                else if ( this.clients[i].projectiles[j].type.indexOf("-rocket") > 0 )
                {
                    this.clients[i].rocketsActive--;
                    rocketPool.ReleaseProjectile(this.clients[i].projectiles[j]);
                }
...
GameServer.prototype.FillState = function(state) {
    for (var i = this.clients.length; i--; ) {
        ...
        for (var j = this.clients[i].projectiles.length; j--; ) {
            this.clients[i].projectiles[j].CalculateDirections(this.clients);
